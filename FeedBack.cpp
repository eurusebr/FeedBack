...
void normarray(vector<double>& a, vector<complex<double>> b);
...
int main(int argc, char** argv)
{
    ...
    boost::normal_distribution<> m_dist(0.0, sigma);
    ...
    // }
    // std::cout << endl;
    normarray(amplitude, complexArray);
    ...
    outFile.write((char*)amplitude.data(), amplitude.size() * sizeof(double));
    outFile.close();
    ...
    {
        // Create a new directory 
        string folder = "./FeedBack_beta(" + to_string(beta) + ")"; 
        struct stat info;
        if( stat( folder.c_str(), &info ) != 0 )
        {
            mkdir(folder.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
            string secfolder = folder + "/alpha(" + to_string(alpha) + ")";
            file = secfolder + "/" + argv[6] + ".bin";
        }
        else if( info.st_mode & S_IFDIR )
        {
            string secfolder = folder + "/alpha(" + to_string(alpha) + ")";
            file = secfolder + "/" + argv[5] + ".bin";
            ofstream outFile(file, ios::binary | ios::trunc);
        }
        ...
        for (int i = 0; i < N; i++)
        {
            AddingArray[i] = AddingArray[i] + complexArray[i];
        }
        normalize(AddingArray, N);
        for (int k = 0; k < N; k++) 
        {
            amplitude[k] = norm(AddingArray[k]);
        }
    }
...
void noise(vector<complex<double>>& a, vector<complex<double>>& b, boost::random::mt19937 & rng, boost::normal_distribution<> & m_dist, double gamma, double beta, double dt, int N)
{
    ...
    {
        rand_array1[l] = sqrt(0.5) * m_dist(rng);
        rand_array2[l] = sqrt(0.5) * m_dist(rng);
    }
    for (size_t i = 0; i < N; ++i) 
    {
        b[i] = -I * sqrt(gamma) * (1.0 - beta * norm(a[i])) * (rand_array1[i] + I * rand_array2[i]) * a[i] - 0.5 * gamma * pow((1 - beta * norm(a[i])),2) * a[i] * dt ;
    }
}
